...
class Classifier():
    ...
    def knn(self, itemVector):
        try:
            neighbors = heapq.nsmallest(self.k, [(self.mahattan(itemVector, item[1]), item) for item in self.data])
...
def testfold(bucketPrefix, dataFormat,k):
    results = {}
    for i in range(1, 11):
...
def main():
    testfold('mpgData/mpgData', 'class\tnum\tnum\tnum\tnum\tnum\tcomment',3)
